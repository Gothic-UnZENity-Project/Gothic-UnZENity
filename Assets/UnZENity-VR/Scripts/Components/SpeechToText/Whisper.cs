...
namespace GUZ.VR.Components.SpeechToText
{
    ...
    public class Whisper
    {
        ...
        public async Task StartExec(AudioClip audioClip)
        {
            ...
            {
                Logger.LogWarning($"Only 30 seconds of recording are supported. {audioClip.samples} provided", LogCat.VR);
                return;
            }
            else if (audioClip.frequency != 16000)
            {
                Logger.LogWarning($"16kHz of recording supported only. But {audioClip.frequency} provided.", LogCat.VR);
                return;
            }
            else if (audioClip.channels != 1)
            {
                Logger.LogWarning($"Only mono is supported. But {audioClip.channels} provided.", LogCat.VR);
                return;
            }
        ...
        async Task InferenceStep()
        {
            ...
            {
                Logger.LogWarning($"Whisper/Inference stuck. Force stop audio inference after {_forcedTimeout}s", LogCat.VR);
                IsTranscribing = false;
                return;
            }
